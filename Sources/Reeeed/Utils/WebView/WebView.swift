...
class _WebViewContainer: UIView, UIScrollViewDelegate {
    ...
    var scrollPosRounded: CGFloat = 0 {
        didSet(old) {
            ...
            if scrollPosRounded < 50 {
                self.scrollDirection = -1 // up
            } else {
                self.scrollDirection = scrollPosRounded > old ? 1 : -1
            }
        }
    }
    ...
    var contentView: WKWebView? {
        didSet(old) {
            ...
            if let view = contentView {
                addSubview(view)
                webviewSubs.insert(view.scrollView.observe(\.contentOffset, options: [.new]) { [weak self] scrollView, _change in
                ...
                webviewSubs.insert(view.scrollView.observe(\.contentSize, options: [.new]) { [weak self] scrollView, _change in
                    self?.onEvent?(.scrollPositionChanged(scrollView.info))
                })
            }
        }
